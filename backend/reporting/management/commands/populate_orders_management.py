    ...
    def handle(self, *args, **options):
        ...
        for i in range(options["n"]):
            ...
            customer = (
                ...
                else customer2 if random.randint(0, 2) == 1 else customer3
            )
            product = (
                product1
                if random.randint(0, 2) == 0
                else product2 if random.randint(0, 2) == 1 else product3
            )
            shipped_name = (
                "John Lennon"
                if random.randint(0, 1) == 0
                else ("Paul McCartney" if random.randint(0, 1) == 0 else "Yoko Ono")
            )
            shipped_address = (
                "Abbey Road 11"
                if random.randint(0, 1) == 0
                else (
                    "Park Avenue 200" if random.randint(0, 1) == 0 else "Via Pairoli 33"
                )
            )
            shipped_city = (
                "New York"
                if random.randint(0, 1) == 0
                else ("London" if random.randint(0, 1) == 0 else "Rome")
            )
            shipped_postal_code = (
                "11000"
                if random.randint(0, 1) == 0
                else ("44100" if random.randint(0, 1) == 0 else "55220")
            )
            shipped_country = (
                "USA"
                if random.randint(0, 1) == 0
                else ("UK" if random.randint(0, 1) == 0 else "Italy")
            )
